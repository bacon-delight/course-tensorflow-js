...
class LogisticRegression {
	...
	gradientDescent(trainingInputs, trainingOutputs) {
		...
		return this.weights.sub(slopes.mul(this.options.learningRate));						// Operation 4
	}
			...
			this.weights = tf.tidy(() => {
				if (this.options.batch) {
					for(let j=0 ; j<batches ; j++) {
						...
						return this.gradientDescent(inputs, outputs);
					}
				}
				else {
					return this.gradientDescent(this.trainingInputs, this.trainingOutputs);
				}
			});
			this.recordCost();
		...
		this.costHistory
			...
			const term2 = this.trainingOutputs
				...
				.matMul(guess
					.mul(-1)
					.add(1)
					.add(1e-7)								// Deal with NaN/Infinity - Never take log of 0, but close to 0
					.log());
